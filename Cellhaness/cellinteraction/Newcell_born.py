...
class SinusoidalPositionEmbedding(nn.Module):
    ...
    def forward(self, t):
        """
        t: 时间步张量，形状为 (B,)
        返回: 时间步的位置编码，形状为 (B, embedding_dim)
        """
        return self.pe[t]
...
class TriangleAttention(nn.Module):
    ...
    def forward(self, h_pair: torch.Tensor, mask=None):
        ...
        # 调整维度: (B, N, N, d) → (B*N, N, d)
        h = h_pair.reshape(B * N, N, d)
        
        # 应用注意力
        attn_out, _ = self.attention(h, h, h)
        
        # 调整回原始维度: (B*N, N, d) → (B, N, N, d)
...
class CellInteractionExtractor(nn.Module):
    ...
    def forward(self, h_pair: torch.Tensor):
        ...
        # 计算通路数
        P = int(torch.sqrt(torch.tensor(crosstalk_flat.shape[-1])).item())
        crosstalk = crosstalk_flat.reshape(B, N, N, P, P)  # (B, N, N, P, P)
...
class CellAtlasDiffusion(nn.Module):
    ...
    def forward(self, z_0, h_single, h_pair, t):
        ...
        # 确保cond和t_emb的维度匹配
        # 我们可以使用线性层将cond的维度映射到与t_emb相同的维度
        if cond_single.shape[-1] + cond_pair.shape[-1] != t_emb.shape[-1]:
            # 创建一个线性层来映射cond的维度
            cond_proj = nn.Linear(cond_single.shape[-1] + cond_pair.shape[-1], t_emb.shape[-1]).to(cond_single.device)
            cond = cond_proj(torch.cat([cond_single, cond_pair], dim=-1))
        else:
            cond = torch.cat([cond_single, cond_pair], dim=-1)
    ...
    def sample(self, z_0_init, h_single, h_pair, n_steps=50):
        ...
        z_0_init: 从Z0Generator获得的初始化，可以是字典或张量
        """
        # 处理字典类型的z_0_init
        if isinstance(z_0_init, dict):
            # 从字典中提取expression和coordinates并拼接
            expression = z_0_init['expression']
            coords = z_0_init['coordinates']
            z_0_init = torch.cat([expression, coords], dim=-1)
...
class CellDenoiser(nn.Module):
    ...
    def forward(self, z_t, cond, t_emb):
        ...
        # 简单地将条件信息添加到输入中
        cond_combined = cond + t_emb  # (B, cond_dim)
        # 将条件信息广播到每个细胞
        cond_combined = cond_combined.unsqueeze(1).repeat(1, N, 1)  # (B, N, cond_dim)
        
        # 连接输入和条件
        input = torch.cat([z_t, cond_combined], dim=-1)  # (B, N, C + cond_dim)
