...
class CandidateDataset(Dataset):
    ...
    def __getitem__(self, index):
        cell, drug, res_class, auc = self.input_df.iloc[index].values
        
        cell_genotype = {}
        cell_genotype['mut'] = np.where(self.cell2mut.loc[cell].values == 1)[0]
        if self.cell2cna is not None:
            cell_genotype['cna'] = np.where(self.cell2cna.loc[cell].values == 1)[0]
        if self.cell2cnd is not None:
            cell_genotype['cnd'] = np.where(self.cell2cnd.loc[cell].values == 1)[0]
