...
from logics_package import smiles_lstm, smiles_vocab, tools, analysis
...
def pretrain(config):
    ...
    for epoch in range(1, config.max_epoch+1):
        ...
        ssplr = tools.SafeSampler(lstm_generator, model_setting['batch_size'])
        ...
        # check the validity
        vacans, invids = analysis.get_valid_canons(decoded_samples)
...
def VanillaGPC_training(config):
    ...
    for epo in range(config.max_epoch+1):
        print("----- epoch: ", epo)
        ssplr = tools.SafeSampler(lstm_agent, config.sampling_bs)
        ...
        vacans, _ = analysis.get_valid_canons(decoded_samples)
